...
// wbModels is the static fallback model list for QoderWork CN. Keys mirror the
// live GET /algo/api/v2/model/list response (verified 2026-09-18). Aliases use
// the qoder/ prefix in AuthAttributes; bare IDs work too. Dynamic refresh via
// the same endpoint replaces this at runtime when an account is present, so a
// stale fallback here only matters before the first successful fetch.
//
// qfmodel (Qwen3.8-Flash) and qmodel_38max (Qwen3.8-Max) are flagged is_free
// upstream; the rest are credit-billed.
func wbModels() []pluginapi.ModelInfo {
	return []pluginapi.ModelInfo{
		{ID: "qfmodel", Name: "Qwen3.8-Flash (free)", ContextLength: 1000000, MaxCompletionTokens: 8192, OwnedBy: providerName, SupportedGenerationMethods: []string{"chat"}},
		{ID: "qmodel_38max", Name: "Qwen3.8-Max (free)", ContextLength: 1000000, MaxCompletionTokens: 8192, OwnedBy: providerName, SupportedGenerationMethods: []string{"chat"}},
		...
		{ID: "q37fmodel", Name: "Qwen3.7-Flash", ContextLength: 1000000, MaxCompletionTokens: 8192, OwnedBy: providerName, SupportedGenerationMethods: []string{"chat"}},
		...
		{ID: "dfmodel", Name: "DeepSeek-Flash", ContextLength: 180000, MaxCompletionTokens: 8192, OwnedBy: providerName, SupportedGenerationMethods: []string{"chat"}},
		...
		{ID: "gfmodel", Name: "GLM-5.3-Flash", ContextLength: 180000, MaxCompletionTokens: 8192, OwnedBy: providerName, SupportedGenerationMethods: []string{"chat"}},
